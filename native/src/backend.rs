...
const BACKEND_NAME: &str = "glama-status-mcp-backend.exe";
const BACKEND_PORT: u16 = 11072;
const BACKEND_TAG: &str = "glama-status-mcp-backend-x86_64-pc-windows-msvc.exe";
const ENV_PORT: &str = "MCP_PORT";
const ENV_HOST: &str = "MCP_HOST";
const ENV_TAURI: &str = "GLAMA_STATUS_TAURI";
...
fn resolve_bundled_backend(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    if let Ok(path) =
        app.path().resolve(BACKEND_NAME, BaseDirectory::Resource)
    {
    ...
    if let Ok(path) =
        app.path().resolve(&resources_path, BaseDirectory::Resource)
    {
        tried.push(path.display().to_string());
        if path.exists() {
            return Ok(path);
...
fn free_port(port: u16) -> bool {
    let img_kill = format!(
        "Stop-Process -Name 'glama-status-mcp-backend' -Force -ErrorAction SilentlyContinue; \
         Stop-Process -Name 'glama-status-native' -Force -ErrorAction SilentlyContinue; \
         taskkill /F /IM glama-status-mcp-backend.exe /T 2>$null; \
         taskkill /F /IM glama-status-native.exe /T 2>$null"
    );
        ...
        if i == 15 {
            let elevated = format!(
                "Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList \
                 '-NoProfile -Command \"Stop-Process -Name glama-status-mcp-backend -Force \
...
pub fn spawn_backend(
    ...
    if !free_port(BACKEND_PORT) {
        let msg = format!(
            "Could not free port {BACKEND_PORT} after 240s"
        );
        log_line(&app, &msg);
        return Err(msg);
    }
    ...
    let mut child = command.spawn().map_err(|e| {
        format!("Failed to spawn {}: {e}", backend_path.display())
    })?;
    ...
    let addr =
        SocketAddr::from_str(&format!("127.0.0.1:{BACKEND_PORT}")).unwrap();
    ...
    thread::spawn(move || {
        for attempt in 0..30 {
            ...
            match TcpStream::connect_timeout(
                ...
                Ok(_) => {
                    ...
                    let _ =
                        app_health.emit("backend-status", "ready");
                    return;
                }
                Err(e) => {
                    log_line(
                        &app_health,
                        &format!(
                            "Backend health check: {e} (attempt {})",
                            attempt + 1
                        ),
                    );
                }
        ...
        log_line(
            &app_health,
            &format!(
                "Backend health check FAILED -- not listening on port {BACKEND_PORT}"
            ),
        );
        let _ = app_health
            .emit("backend-status", "error: backend not reachable");
    });
